...
export const SignUpContent = (props) => {
    ...
    const submitHandler = async(e) => {
        ...
        try {
            ...
            setSucessMessage('Registration sucessfully')
            setErrorMessage(null)
            props.setToLogin(false)
        } catch (error) {
    ...
    return (
        <form onSubmit = {submitHandler} >
            <div>
                {sucessMessage && <p className='text-green-900 font-semibold italic'>{sucessMessage}</p>}
                {errorMessage && <p className='text-red-900 font-semibold italic'>{errorMessage}</p>}
            </div>
